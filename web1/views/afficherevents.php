							...
							<!--search-box-->
								<div class="w3-search-box">
									<form action="calculer.php" method="post">
										...
										<input type="submit" value="recherche" onclick="rechercherListeEvents()" name="recherche">					
									</form>
								</div><!--//end-search-box-->
...
if(isset($_POST['tri']))
{
	$listeEvents=$db->query("SELECT * FROM event ORDER BY event.`date` ASC");
}
else if(isset($_POST['tria']))
{
	$listeEvents=$db->query("SELECT * FROM event ORDER BY event.`date` DESC");
	
}
else $listeEvents=$event1C->afficherEvents();
...
?>
	...
	<div class="crud">
					<form method="post">
						<button type="submit" name="tri" class="btn btn-primary"  >asc</button> 
					  	<button type="submit" name="tria" class="btn btn-primary" >desc</button>
						</form>
...
foreach($listeEvents as $row){
	...
	<td><?PHP echo $row['nbmax']; ?></td>
	<td><?PHP echo $row['description']; ?></td>

	<td><form method="POST" action="supprimerEvent.php">
	<input style="border-radius: 10px" class="btn btn-primary" type="submit" name="supprimer" value="supprimer">
	<input type="hidden" value="<?PHP echo $row['id']; ?>" name="id">
	</form>
	</td>
	
	<td><a href="modifierevent.php?id=<?PHP echo $row['id']; ?>">
	<input  class="btn btn-primary" style="border-radius: 10px" type="submit" name="modifier" value="modifier"></a></td>
	</tr>
	<?PHP
